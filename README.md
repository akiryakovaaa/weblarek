...
В приложении используется один презентер, который отвечает за логику работы единственной страницы магазина.

Презентер реализован в файле `main.ts` без отдельного класса.  
Он создаёт экземпляры моделей данных и компонентов представления, подписывается на события от моделей и View, а в обработчиках:

- вызывает методы моделей для изменения данных;
- вызывает методы компонентов представления для обновления интерфейса;
- управляет модальным окном (открытие/закрытие и подстановка нужного компонента внутрь).

Презентер сам не генерирует события — он только обрабатывает события, которые приходят от моделей данных и слоя Представления, согласно паттерну MVP.
